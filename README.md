...
* **La honra, on screen — and rewarded.** Once you earn your first *gesta*, the
  room-name row shows **`honra n/7`**; and the very moment a new deed lands, a
...
(`build_blit.py`) splits the Koala into the three chunks the loader `BLOAD`s
...
* **C128** (`--detail`): the art is **RLE-compressed to ~10 KB** (1.9×, with a
  ...
  `$A700` — bank-0 RAM above the program, safe because BASIC 7.0 keeps all
...
* **C64** (`--c64disk`): the blob is bulk-`LOAD`ed **once at boot** as three
  ...
  the ROMs into RAM), 27-32 at `$C100`. The per-room blit banks all RAM in
...
under true-drive emulation. The single-file `ELCID.PRG` keeps the compact
resident art for cartridge or one-file use.
...
the crossed swords Tizona & Colada, shown only when all six honour *gestas* are
found) and **DERROTA** (the sword thrust point-down into the burial mound, the
...
```
 ...
 | PUERTA DE BURGOS                     |  row 10     room name
 | burgos cerrada so pena de los ojos.  |  rows 12-13 description
...
*honra*). A full solution — and the six honour *gestas* — is in
...
| C64 lite (`ELCID.PRG`) | 403 jiffies (~6.7 s) | **248 (~4.1 s)** | 1.6× |
...
2. **`cidsim.py`** — a Python reference engine that runs the *exact same logic*
   ...
   winnable** — gathering all six honour *gestas* to prove the **legendary ending
   is reachable** — and checks every lose condition fires. (`*** VICTORY ***`.)
3. **`scenes.py` / `c64.py`** — the 32 PETSCII scenes, authored once and used to
   emit **both** the BASIC art data **and** the PNG previews above (a faithful
   C64-palette simulator).
...
spec, and the BASIC is provably legal v2.0.* The composite screenshots above were
rendered by the same simulator to verify the on-screen layout (no overflow, no
scroll).
