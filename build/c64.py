...
# ---------------------------------------------------------------------------
# A hand-authored 8x8 uppercase font, so the screen previews can draw text on a
# machine with no C64 chargen ROM (Debian/Ubuntu ship VICE without the ROMs, and
# this repo is built and verified without them).  screensim.py uses the real
# chargen when one is installed and falls back to this.
# ---------------------------------------------------------------------------
FONT = {}
def g(ch, art):
    rows = art.split("/")
...
def text_glyph(sc):
    """8 rows for a screen code 0..63, from the fallback font."""
    ch = " " if sc == 32 else (chr(96 + sc) if 1 <= sc <= 26 else chr(sc))
